...
2. 💿 [ Telepítés ](#installation)
    ...
    2. 📦 [ Adatbázis indítása ](#installation--db)
    3. 📦 [ Adatbázis feltöltése ](#installation--seeder)
...
A programhoz tartozó MongoDB noSQL adatbázishoz szükséges a Docker és a docker compose telepítése. Ez történhet a Docker Desktop letöltésével, vagy telepíthető CLI-ből is.
...
<a name="installation--db"></a>
# 📦 Adatbázis indítása

A MongoDB adatbázis létrehozásához/indításához a következő parancsot szükséges futtatni:
```bash
docker compose up -d
```

Leállítani pedig az alábbival tudjuk:
```bash
docker compose down
```
...
Ami az alábbi kimenetet adja:
```bash
./scripts/docker/../../.env
Copying over events.json file to MongoDB container..
Successfully copied 20.2kB (transferred 22kB) to mongodb:/tmp/events.json
Importing events.json via docker exec & mongoimport..
2026-04-29T10:34:51.593+0000	connected to: mongodb://localhost/
2026-04-29T10:34:51.593+0000	dropping: `bookings.events`
2026-04-29T10:34:51.600+0000	20 document(s) imported successfully. 0 document(s) failed to import.
Deleting events.json from container..
Events collection has been seeded successfully.
```
